...
function Change(props, state) {
  return (
    <div className='dashboard-change' onClick={() => { props.selectChange(props.change.get('id')) }}>
...
function DashboardHeader(props, state) {
  return (
    <div id='dashboard-header'>
      ...
      <div className='dashboard-error'>{props.state.get('error')}</div>
...
export default class Dashboard extends React.Component {
  ...
  componentDidMount() {
    if (!this.props.state.get('dashboardError') && !this.props.state.get('changes')) {
